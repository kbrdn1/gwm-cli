...
- The sidebar `Working Tree` pane bottom-right footer now breaks the changed-file tally into colour-coded nerdfont counts — created (green), modified (yellow), deleted (red) — derived from `git status --short`, with each segment shown only when its count is non-zero (a clean tree shows no footer count).
