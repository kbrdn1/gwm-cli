...
- The sidebar `Working Tree` pane bottom-right footer now breaks the changed-file tally into colour-coded nerdfont counts — created (green), modified (yellow), deleted (red) — derived from `git status --short`, with each segment shown only when its count is non-zero (a clean tree shows no footer count). Each file row is recoloured by the same change category so a row's colour matches the footer segment it's counted in (created → green, modified → yellow, deleted → red); the previous staged-vs-worktree cyan column split is retired.
- The sidebar `[2] Status` pane now shows a `Diff +<ins> -<del>` line (insertions green, deletions red) for the branch versus its base trunk, using the three-dot `git diff --shortstat <base>...HEAD` semantics so the figure matches the base `gwm pr` would target. The line is hidden when no base resolves, when HEAD is the trunk itself, or when the branch has no committed diff yet.
...
- The worktree table renders Issue/PR as two slots `●/●`, `●/-`, `-/●`, or `-/-` (left = Issue, right = PR; `-` means no link; the main worktree keeps its `★`). The Issue/PR pane now leads each line with a nerdfont dev glyph and badges the link source (` auto ` / ` detected `) and GitHub state (` open ` / ` closed ` / ` draft ` / ` merged `) as version-style reverse-video chips.
...
- An auto-detected PR (`gh pr list --head <branch>`) is now persisted to a dedicated `branch.<name>.gwm-pr-detected` git-config key on the `F` refresh, so the worktree table can colour its PR pastille on every row without a PR-detection shell-out per row. Issue/PR titles and states are persisted alongside linked numbers (`gwm-issue-title`, `gwm-issue-state`, `gwm-pr-title`, `gwm-pr-state`, `gwm-pr-detected-title`, `gwm-pr-detected-state`) after successful status fetches, allowing the pane to show cached titles, status chips, and state-coloured icons on startup before or during a new fetch. An explicit `gwm link --pr` still wins, and a vanished detection clears the stored key/title/state.
...
- The Worktrees pane Issue/PR pastilles now switch to loaded GitHub state colours across all linked rows after a worktree refresh, so closed issues/PRs no longer fall back to the linked-only colours after relist.
- The TUI now kicks off an initial Issue/PR status refresh for all linked worktrees at startup when a GitHub remote is available, and worktree refreshes persist refreshed Issue/PR state per branch so markers survive closing and reopening the app.
