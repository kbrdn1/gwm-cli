...
- **GitHub workflow**: branches matching `<type>/#<N>-<slug>` auto-link to their issue (with ephemeral PR auto-detection); `gwm new` opens an issue from a template then spins up the worktree and `gwm create --issue <N>` ([#617](https://github.com/kbrdn1/gwm-cli/issues/617)) does the same for an issue already filed, deriving the branch type from its labels and the slug from its title; `gwm pr` renders the PR body; `gwm review <PR#>` ([#308](https://github.com/kbrdn1/gwm-cli/issues/308)) pulls an existing PR (including one from a fork) into an isolated worktree (fetch + link), the inbound counterpart to `gwm create` (safe-by-default: bootstrap/hooks are opt-in via `--bootstrap`, since a fork PR's setup commands are arbitrary code); live status surfaces in the TUI sidebar via `gh`.
...
- **A listing that stops degrading with use** ([#633](https://github.com/kbrdn1/gwm-cli/issues/633)): `gwm list` scans its worktrees in parallel, and `gwm doctor` reports the `branch.<name>.gwm-*` keys a deleted branch left behind, which `--fix` drops from `.git/config`. libgit2 reparses that file on every `Repository::open` and snapshots it again for each `statuses()`, so a config nobody prunes is a tax the listing pays once per worktree: 184 ms becomes 100 ms on a repo grafted to 1434 config lines, with 8 worktrees (the purged file keeps its empty section headers, so that figure is measured rather than interpolated).
...
- **Mouse** ([#624](https://github.com/kbrdn1/gwm-cli/issues/624)): click a worktree row to select it, a pane to focus it, a row in any modal listing to pick it, a Settings tab to switch to it, or the `✕` in a modal's corner to close it; the wheel scrolls whatever is under the pointer. The header carries a `▤` and a `⚙` opening the Command Logs and Settings panels — until now the only sign those panels existed was the help overlay. Reading the mouse and letting the terminal select text are mutually exclusive, so `M` switches between the two, `[tui] mouse = false` starts on the other side of it, and the header says which state you are in; `Shift`+drag reaches the terminal's own selection without giving the click up, where the terminal's own setting allows it. gwm keeps its side small by asking for press tracking only, never the drag and motion reporting most TUIs turn on with it.
