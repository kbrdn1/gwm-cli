...
- **Per-worktree notes** ([#515](https://github.com/kbrdn1/gwm-cli/issues/515)) — `N` opens the selected worktree's note in `$EDITOR` and the table marks the rows that carry one. Plain Markdown under `<main-checkout>/.git/gwm/notes/`, so it is greppable with gwm shut down, never committed, and it survives `gwm remove`; `gwm note show [slug]` reads it back and the `--format=json` rows carry it. `gwm doctor` reports a note whose branch is gone.
