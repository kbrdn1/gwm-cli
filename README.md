...
- **Mouse** ([#624](https://github.com/kbrdn1/gwm-cli/issues/624)): click a worktree row to select it, a pane to focus it, a row in any modal listing to pick it, a Settings tab to switch to it, or the `✕` in a modal's corner to close it; the wheel scrolls whatever is under the pointer. The header carries a `▤` and a `⚙` opening the Command Logs and Settings panels — until now the only sign those panels existed was the help overlay. gwm asks only for the two mouse modes it reads, so `Shift`+drag has a chance of reaching the terminal's own text selection; `M` releases the mouse outright for the terminals where it does not (Ghostty 1.3.1 among them).
