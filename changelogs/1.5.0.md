...
have sent an authenticated call — and whatever token the environment carries —
to whatever host a cloned repo's `origin` happened to name. Authorising a
...
One caveat stated plainly: the GitLab backend's contract was verified against
`glab` 1.109.0's own source and documentation, not against a running GitLab
instance. Field reports on a real instance, self-hosted especially, are
welcome.
