...
The flake serves `x86_64-linux`, `aarch64-linux` and `aarch64-darwin`. Intel macOS (`x86_64-darwin`) is not among them: the nixpkgs this flake pins dropped that platform, so use the prebuilt archive, `cargo binstall` or `cargo install` there.
