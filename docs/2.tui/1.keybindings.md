...
prompt; Codex's thread name — `session_index.jsonl`, so a rename shows —
...
| `f`            | filter the list (`filter`) — live substring query, Enter opens the highlighted match. While the filter is typing, printable keys feed the query (palette convention, same as the attach prompt); the rebindable verbs apply in list mode |
| `Esc` / `q`    | close (`close`)                                                         |
