...
**`M` is the way.** Reading the mouse and letting the terminal select text are
mutually exclusive: while gwm is reading, a drag belongs to gwm and the
terminal does not run its own selection. `M` switches reporting off until
pressed again, the header carries a ` mouse off · M ` chip while it is off so
the mode is never silent, and the setting survives a trip through lazygit, an
`exec` run or a review launcher — coming back from a fullscreen child does not
quietly hand the mouse back to gwm.

If that trade is the wrong way round for you, `[tui] mouse = false` in
`.gwm.toml` starts every session with the mouse released, and `M` still turns
it on when you want to click. See
[Configuration → `mouse`](/configuration/gwm-toml#mouse).

gwm keeps its side of the trade as small as it can:
it asks for **press tracking only** (`1000`), never the drag and motion
reporting (`1002` / `1003`) that most TUIs turn on with it. Nothing here reads
a drag, so nothing here asks for one.

`Shift`+drag is the convention for reaching past a TUI to the terminal's own
selection, and it is worth trying — but it is honoured by the terminal, not by
the application, and not every one honours it. **Measured on Ghostty 1.3.1
under herdr: it does not.** Terminals that expose it name it something like
Ghostty's `mouse-shift-capture` or iTerm2's "Terminal may report mouse events".
