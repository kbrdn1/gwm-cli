...
> [Configuration → `[tui.keys]`](/configuration/gwm-toml#tui-keys).
...
| `:`         | open the [command palette](/tui/keymap-and-palette#command-palette) (rebindable action `command_palette`) |
...
| Key                  | Action                                                                          |
|:---------------------|:--------------------------------------------------------------------------------|
| `←` / `→` / `Tab`    | move focus between `[ Confirm ]` and `[ Cancel ]` (defaults to Cancel)          |
| `Enter`              | activate the focused button (Confirm → delete · Cancel → dismiss)               |
| `y`                  | confirm (classic) or arm the countdown (when `p` is armed — see [countdown](/tui/confirm-countdown)) |
