...
While the mouse is reported, a drag belongs to gwm rather than to the
terminal's own selection, so the usual click-and-drag to copy a path does not
select anything. Two ways out:

- **`Shift` + drag.** Most terminals (iTerm2, Ghostty, kitty, WezTerm,
  Alacritty, GNOME Terminal, Windows Terminal, Terminal.app) bypass mouse
  reporting while `Shift` is held and do their own selection. This is a
  convention rather than a guarantee, so it may be a different modifier, or
  absent, on a terminal that does not follow it.
- **`M`.** Turns mouse reporting off outright until pressed again. The status
  bar says so, and the setting survives a trip through lazygit, an `exec` run
  or a review launcher — coming back from a fullscreen child does not quietly
  hand the mouse back to gwm.

Yanking is still the direct route: `Y` copies the worktree path, `y` the
branch name, `w` the worktree name.
