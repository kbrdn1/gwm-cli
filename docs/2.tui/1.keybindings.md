...
| `N`         | open the selected worktree's note in `$EDITOR` (`edit_note`), see [notes](#notes-n)               |
...
`N` opens the selected worktree's note in `$EDITOR`. gwm suspends, the editor takes the terminal, and the TUI comes back when the editor exits, the same handoff `o` uses in [`mode = "editor"`](/tui/open-dispatch): `editor_cmd` in `.gwm.toml`, then `$EDITOR`, then `vi`.
...
> An *ended* session cannot always be observed from artefacts alone: only
...
**issue** instead, on the same shell minus the PR-only blocks. Nothing is
fetched on top of what the status refresh already asks for, so the view is
as fresh as the last `f`. With neither side fetched, nothing opens and the
status bar names the way out.
...
sections are absent rather than shown empty.
