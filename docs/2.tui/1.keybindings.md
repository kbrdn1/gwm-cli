...
does, which is also where its refusals come from: a zellij tab takes no
command and neither herdr level does, and the status bar names whichever said
no. The resume command per backend is
