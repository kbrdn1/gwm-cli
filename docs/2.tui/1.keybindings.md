...
While the mouse is reported, a click belongs to gwm rather than to the
terminal's own selection, so click-and-drag to copy a path may not select
anything.

**`M` is the answer that always works.** It turns mouse reporting off outright
until pressed again; the status bar says so, and the setting survives a trip
through lazygit, an `exec` run or a review launcher — coming back from a
fullscreen child does not quietly hand the mouse back to gwm.

`Shift`+drag is worth trying first, and gwm does what it can to let it through:
it asks only for the two mouse modes it actually reads (press and wheel) and
switches off the drag / motion reporting `EnableMouseCapture` would otherwise
turn on, which is what a terminal keys its "the application wants the whole
mouse" behaviour off. Whether `Shift` then bypasses reporting is the
terminal's decision, and not every one of them makes it — **Ghostty 1.3.1 does
not** (measured). Check your terminal's own setting before assuming it should:
Ghostty has `mouse-shift-capture`, iTerm2 has "Terminal may report mouse
events".
...
does — is their decision before it is the terminal's. If drag and click
disagree with what this page says, check the multiplexer's mouse setting first
(`set -g mouse` under tmux).
