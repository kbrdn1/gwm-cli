...
Both columns sit one gap past the **longest row**, not out at the overlay's
border. The overlay is 90% of the screen, so a border-flush column would put
the letter a hundred cells from the name it describes, which is the opposite
of the fixed column the listing needed. Their offset therefore depends on the
listing, not on the terminal: widening the window does not move them.
