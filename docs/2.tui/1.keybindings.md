...
session, most recent first, with the agent name, its freshness (**active** =
artefact activity in the last 5 minutes, **idle** otherwise) and a
human-readable last-activity time. Detection reads each agent's on-disk
session artefacts (Claude Code, Codex, opencode, Mistral Vibe) — no process
