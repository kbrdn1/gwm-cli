...
human-readable last-activity time and the session's **name** (its first
prompt, or Vibe's recorded title) — falling back to the full session id when
the artefacts carry no name. Detection reads each agent's on-disk session
...
| `a`            | pin the selected session (`attach`)   |
| `d`            | remove the pin (`detach`)             |
| `Esc` / `q`    | close (`close`)                       |

> An *ended* session cannot always be observed from artefacts alone: only
> Mistral Vibe records an explicit end marker. A session that just exited may
> read as **active** for up to 5 minutes. Process-level liveness is a
> deliberate non-goal for now (#408).
