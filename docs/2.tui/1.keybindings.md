...
![The `?` in-app Keybindings overlay](./_assets/keybindings.png)
...
| `M`         | release / recapture the mouse (`toggle_mouse`), see [mouse](#mouse-m)                             |
| `gg`        | jump to the first worktree (`top`)                                                                |
| `G` / `End` | jump to the last worktree (`bottom`)                                                              |
| `←` / `→`   | fold / unfold the highlighted row's repo group (`collapse_group` / `expand_group`), workspace mode only, see [repo groups](#repo-groups-workspace-mode) |
...
The TUI reads mouse events ([#624](https://github.com/kbrdn1/gwm-cli/issues/624)).
Nothing has to be enabled; every terminal that reports the mouse works.

| Gesture                        | Effect                                                                 |
|:-------------------------------|:-----------------------------------------------------------------------|
| click a worktree row           | select it, and focus the worktrees pane                                |
| click a pane                   | focus it, the same as `Tab` / `1` / `2`                                |
| click a row in any modal listing | move that listing's cursor to it                                     |
| click a sidebar section title  | open that section full size (`Issue / PR` → `I`, `Agents` → `a`, `Working Tree` → `W`, `Recent Commits` → `c`) |
| click a create / rename form field | focus it                                                          |
| click the `‹` / `›` of the branch-type selector | step it back / forward                              |
| click `Confirm` / `Cancel`     | focus that button and activate it                                      |
| click a Settings tab           | switch to it                                                           |
| click the `▤` / `⚙` in the header | open the Command Logs / Settings panel, the same as `3` / `4`       |
| click the `✕` in a modal's corner | close it, the same as `Esc`                                          |
| click the Working Tree counts  | open the full-size Working Tree listing, the same as `W`               |
| wheel                          | scroll or move the selection of whatever is **under the pointer**      |

The wheel follows the pointer rather than the focus: pointing at a pane is how
a pointer says which pane it means. Focus is still what `Tab` and the digits
set, and `j` / `k` still follow it.

### getting the terminal's text selection back

Reading the mouse and letting the terminal run its own text selection are
mutually exclusive: while gwm is reading, a drag belongs to gwm. There are
three ways to have both, in the order you probably want them.

**1. `Shift`+drag.** The middle ground, and the one that gives nothing up: the
gesture reaches past the TUI to the terminal's own selection while every click
still works. It is honoured by the terminal rather than by the application, so
the terminal's setting decides — and a setting that turns it off is easy to
have without meaning to. Verified on Ghostty 1.3.1 under herdr:
`mouse-shift-capture = false` (the default) selects text, `always` sends the
gesture to gwm instead and nothing is selected. iTerm2 spells the same knob
"Terminal may report mouse events".

**2. `M`, for the moment.** Switches reporting off until pressed again. The
header carries a ` mouse off · M ` chip while it is off, so the mode is never
silent, and the setting survives a trip through lazygit, an `exec` run or a
review launcher — coming back from a fullscreen child does not quietly hand the
mouse back to gwm.

**3. `[tui] mouse = false`, for good.** Starts every session with the mouse
released, and `M` still turns it on when you want to click. The right choice if
you copy paths with the pointer more often than you click rows. See
[Configuration → `mouse`](/configuration/gwm-toml#mouse).

gwm keeps its own side of the trade as small as it can: it asks for **press
tracking only** (`1000`), never the drag and motion reporting (`1002` / `1003`)
that most TUIs turn on with it. Nothing here reads a drag, so nothing here asks
for one.

**Under a multiplexer, the multiplexer arbitrates.** tmux, zellij and herdr
each have their own mouse handling, and what reaches gwm — and what a drag
does — is their decision before it is the terminal's. If a gesture disagrees
with this page, that setting is the first place to look (`set -g mouse` under
tmux). Trying the same gesture outside the multiplexer is what tells the two
apart.

Yanking bypasses the whole question: `Y` copies the worktree path, `y` the
branch name, `w` the worktree name, and they route through OSC52 over SSH.
...
`Space` was the sidebar layout cycle before [#484](https://github.com/kbrdn1/gwm-cli/issues/484); that verb now lives on `z`. Both defaults are one `[tui.keys]` line away (`cycle_sidebar_layout = ["Space"]`, `toggle_select = ["z"]`). If your `.gwm.toml` binds a chord *starting* with `z` (say `top = ["z z"]`), that is now a prefix conflict against a shipped default and is refused at load time: rebind it, or move `cycle_sidebar_layout` somewhere else.

## repo groups (workspace mode)

In [workspace mode](/cli/reference#workspace-mode-global---workspace-issue-36)
the table lists every repo's worktrees in one list, each repo's main worktree
first. Since [#680](https://github.com/kbrdn1/gwm-cli/issues/680) each repo is
an accordion whose header is that main row: `←` folds the group the cursor is
in down to its header, `→` unfolds it. The header carries a chevron in its age
cell (`▾` open, `▸` folded; the slot is free because a trunk branch has no
age), and a folded header paints in the `muted` role so a folded repo reads as
folded at a glance. A repo with a single worktree heads nothing, so it shows no
chevron and cannot be folded.

Folding moves the cursor onto the header, since the row it sat on is usually
one of the rows the fold hides, and drops any mark on a hidden row: a mark out
of sight is a row `d` would delete without showing it. An active `/` filter
overrides the fold (the query matches worktree names, so it can match a linked
row while filtering its header out), hides the chevrons while it lasts, and
makes `←` / `→` inert: a fold is never recorded under a query, and clearing
the query drops the marks the returning fold hides. The fold state is
session-only: nothing persists it, and every group starts open.
Both keys are no-ops in single-repo mode, and rebind as `collapse_group` /
`expand_group` under `[tui.keys]`.
...
`N` was unbound before [#515](https://github.com/kbrdn1/gwm-cli/issues/515). If your `.gwm.toml` binds a chord *starting* with `N` (say `top = ["N x"]`), that is now a prefix conflict against a shipped default and is refused at load time: rebind it, or move `edit_note` somewhere else.
...
view, each led by its own glyph. `L` flips the edit layer between the project
`.gwm.toml` and the user-global config; the layer selector decides which file
an edit writes.

Since [#623](https://github.com/kbrdn1/gwm-cli/issues/623) every tab reads as
two columns: what a row is on the left, what it is set to on the right, aligned
against the panel's own edge so the panel's width is used rather than left
empty beside a content-sized block. On a panel too narrow for both the gap
falls back to a minimum and the row pans sideways, as it always did.

On the editable tabs the shape of a value says what kind it is:

| Value reads | Kind | Keys |
|:--|:--|:--|
| `‹ value ›` | a choice, and the markers double as the hint that it cycles | `←` / `→`, or `Space` / `Enter` |
| `[✓]` / `[ ]` | a boolean, which is a two-value cycle | `←` / `→`, or `Space` / `Enter` |
| plain text or a number | typed, not cycled | `Space` / `Enter` opens the edit buffer |
| `(unset)` | an optional text field nobody has set | `Space` / `Enter` |

The `TUI` tab, the long one, groups its sixteen rows under labelled rules:
**Appearance**, **Sidebar**, **Editing**, **Open**, **Multiplexer**,
**Browser**, **Timing**. The `Keys` and `All` tabs wear the same rules over
their `[global]` / `[modal.<context>]` scopes and their `[table]` sections, and
put their binding and their resolved value in the same right-hand column.

| Key            | Action                                                              |
|:---------------|:-------------------------------------------------------------------|
| `Tab` / `BackTab` | next / previous tab                                             |
| `↑` / `↓`      | select a field / binding (scrolls on the `All` tab)                |
| `←` / `→`      | adjust the selected choice one value back / forward (pans the `All` tab, and does nothing on `Keys`) |
...
The title carries the changed-file count, `Working Tree (27)` (issue
[#622](https://github.com/kbrdn1/gwm-cli/issues/622)). It counts files, not
rows, so it agrees with the footer: directory rows, the `… N more` overflow
notice and the `✓ clean` sentinel are rows without being changed files. It is
left off while the listing is still being read, since a `(0)` there would be a
...
Further right still, in a column of its own, sits the `M` / `A` / `D` / `?`
status letter (issue [#622](https://github.com/kbrdn1/gwm-cli/issues/622)).

The two columns yield in a fixed order. The letter is paid for first, so a
narrowing terminal drops the `+120 -34` counts and keeps the letter; narrow
enough and both go, but the file name never does. `+120 -34` says how much a
file changed, the letter says what happened to it, and a row that no longer
says what it is has lost its subject rather than a detail.
...
session opens the overlay with an explicit *no agent session found* row
...
The overlay reads as two columns since
[#623](https://github.com/kbrdn1/gwm-cli/issues/623): the description on the
left, the chord for it right-aligned against the overlay's own edge, under the
same labelled section rules the Settings panel wears, their separator muted so
it marks a group without competing with the name in it. Chords render as flat
accent-bold glyphs and all colours follow the resolved `[theme]`.

The chords win when the two columns compete for a narrow overlay: a
description that will not fit is truncated with an ellipsis, because a keys
reference that shows no keys is not one. The descriptions were cut to fit
instead, so at 80 columns, the narrowest terminal this is used on, nothing is
truncated; a test pins that, so a new binding with a long description fails
rather than quietly shortening every other row.
