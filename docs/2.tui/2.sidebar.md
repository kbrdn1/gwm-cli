...
- `Space` (`cycle_sidebar_layout`) cycles the layout `auto → side-by-side → stacked → auto`. `auto` is the width-driven default above; the other two pin the orientation regardless of width.
