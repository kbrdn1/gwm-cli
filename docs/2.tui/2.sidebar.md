...
- **`Agent`** — the most recently active AI-agent session attached to this worktree (`claude` / `codex` / `opencode` / `vibe`), with its freshness (**active** / **idle**) and a `+N` count when more sessions matched. Hidden when no session is detected. Press `a` for the full list in the [agent sessions overlay](/tui/keybindings#agent-sessions-overlay-a). Added by [#408](https://github.com/kbrdn1/gwm-cli/issues/408).
