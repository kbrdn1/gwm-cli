...
- The `M` / `A` / `D` / `?` **status letter rides a right-aligned column of its
  ...
  It keeps its change-category colour. The column sits one gap past the
  longest row rather than against the pane's border: the sidebar is stacked
  by default, so on a wide terminal a border-flush letter would land a
  hundred cells from its name.
