...
![The details sidebar: Status, Issue·PR, Working Tree, Recent Commits](./_assets/sidebar.png)
...
![Side-by-side layout (wide terminal)](./_assets/side-by-side.png)
![Stacked layout (narrow terminal)](./_assets/narrow.png)
...
- **Single-child directory chains collapse** (`src/tui/` shown as one row, then
  the files under it).
- **Directory rows lead with a `▾` disclosure caret** and carry no file glyph
  (issue [#622](https://github.com/kbrdn1/gwm-cli/issues/622)), so the two
  levels are told apart by shape rather than by indentation alone.
- Each file row carries an **extension-driven file-type icon** and its name.
- The `M` / `A` / `D` / `?` **status letter rides a right-aligned column of its
  own** (issue [#622](https://github.com/kbrdn1/gwm-cli/issues/622)) rather
  than leading the name, so a long change set reads as one column of letters.
  It keeps its change-category colour, and it is pinned to the pane's right
  edge at every width: the letter is priced far below the `+N -M` column, so
  a pane too narrow to seat the counts still seats the letter.
- Inside the letters, the `+N -M` **line counts** of each file, the same
  column the full-size overlay carries (issue
  [#592](https://github.com/kbrdn1/gwm-cli/issues/592)). A directory, an
  untracked file and a binary file carry none. They are the first thing a
  narrowing pane drops, the letter the last.
