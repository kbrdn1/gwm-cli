...
![The agent sessions overlay: one row per session with agent, freshness, last activity and name](./_assets/agent-sessions.png)
...
- **The `a` overlay**, pictured above: every session attached to the selected worktree, most recent first, one row per session with the agent, its freshness, a human-readable last activity and the session name. A worktree with no session opens an explicit *no agent session found* row rather than a blank modal.
