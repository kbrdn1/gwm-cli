...
![gwm TUI: worktree table and details sidebar](./_assets/hero.png)
...
- **Modals** all share one frame, and since [#594](https://github.com/kbrdn1/gwm-cli/issues/594) that frame follows `[tui] layout` like every other surface. Compact: a filled title band on the first row, the hints on a quiet `section_bg` band under the last one, a blank row between each band and the content, no rules on any side, and the list behind it darkened so the panel still reads as one. Bordered: a rounded border carrying a bold themed title in its top rule. Either way the box is sized to its content rather than to a fixed percentage of the screen. The title moved into the rule in [#549](https://github.com/kbrdn1/gwm-cli/issues/549): it used to be a centred row inside the frame followed by a blank spacer, so every overlay is two rows shorter.

### layout

`[tui] layout` ([#545](https://github.com/kbrdn1/gwm-cli/issues/545)) chooses how panes and sidebar sections are framed. **`"compact"` is the default**: no box rules, one filled header line per section. The title keeps its bracketed keybinding and goes uppercase, the counter sits at the right of that same line, a `muted` rule marks the boundary between the two panes, and the worktrees pane sizes itself to its row count rather than reserving its share of the split. Focus reads on the header fill: the active pane takes a darkened `accent` band with dark bold text on it, while an inactive one keeps its name in `accent` on the quiet `section_bg` fill rather than dimming it ([#605](https://github.com/kbrdn1/gwm-cli/issues/605)). `[tui] dim_unfocused` additionally dims the inactive pane's body, in either layout, off by default.
...
Modals follow the same knob since [#594](https://github.com/kbrdn1/gwm-cli/issues/594): compact spends a filled title band and a `section_bg` footer band instead of the four rules, two rows and four columns less per overlay, and darkens what it floats over to make up for the missing boundary. Configuration and the `section_bg` theme role are documented under [`.gwm.toml`](/configuration/gwm-toml#layout).
