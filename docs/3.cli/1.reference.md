...
A note is plain Markdown stored at `<main-checkout>/.git/gwm/notes/<branch>.md`, written from the TUI's `N` (which opens it in `$EDITOR`) or by editing the file directly. It is never committed, it survives `gwm remove`, and it is readable from the main checkout. See [TUI → notes](/tui/keybindings#notes-n).
