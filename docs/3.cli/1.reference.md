...
## `gwm doctor [--format=text|json] [--fix]`

Run 11 health checks; report each with `✓ / ! / ✗`; exit `0 / 1 / 2`. Designed for CI and pre-commit hooks. See [Integrations → `gwm doctor`](/integrations/doctor) for the per-check breakdown.

![`gwm doctor`: the checks with their `✓ / ! / ✗` status](../5.integrations/_assets/doctor.png)
...
`--fix` (issue #633) is the only remedy any check carries, and the only thing on this
page that **writes**: it drops the `branch.<name>.gwm-*` keys a deleted branch left
behind, from `.git/config` and nothing else. A plain `gwm doctor` never writes. It runs
before the report, so the checks printed under it show the repaired state, and its own
line is text-only so `--format=json` stays parseable. Keys reached through
`include.path` live in a file gwm does not rewrite: those are reported as survivors
rather than counted as purged.
...
- **Repo groups fold** ([#680](https://github.com/kbrdn1/gwm-cli/issues/680)): in the TUI each repo is an accordion headed by its main worktree row. `←` folds the highlighted row's group down to that header, `→` unfolds it; the header carries a `▾` / `▸` chevron in its age cell and paints `muted` while folded. See [repo groups](/tui/keybindings#repo-groups-workspace-mode).
