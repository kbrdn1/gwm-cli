...
daemon and `gwm statusline`.

```bash
gwm agents                       # sessions per worktree: agent, freshness, id
...
gwm agents detach feat-42        # back to pure auto-detection
```
...
regardless of where it was started. One pin per worktree (attach again to
replace it), stored in git branch config (`gwm-agent-pin`), never committed.
Attaching an id detection cannot resolve fails with exit 1; a pin whose
artefacts later disappear degrades silently back to detection. Detached-HEAD
worktrees cannot hold a pin.
