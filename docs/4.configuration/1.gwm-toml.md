...
Two things a derived parser still cannot recover, and `gwm doctor` / `gwm config validate` name both:

1. **A pattern that cannot be compiled.** Two placeholders written back to back leave the parser no boundary to split on. `{issue}{desc}` writes `42123-x` from `42` and `123-x`, which reads back as `4212` and `3-x`; `{desc}{issue}` is worse, because `a12` is what both `a` + `12` and `a1` + `2` produce, so no parser can be right. Both are refused with a message naming the fix. Note the rule is only about *adjacency*: a separator drawn from the left placeholder's own charset is fine, so `{desc}-{issue}` round-trips (an issue number cannot contain the `-`, so there is exactly one valid split).
2. **A segment the pattern never writes.** `{type}/#1-{desc}` freezes the issue number, `feat/#{issue}-{desc}` hardcodes the type. Both create worktrees perfectly well, and both mean gwm has nothing to read that segment out of. The warning says which placeholder to add.
...
| `{type}-{issue}-{desc}` | slash-less, and unambiguous: `-` is in neither the type list nor `\d+` |
...
| `{desc}-{issue}` | see the adjacency note above |

**Refused as unreadable**, with an error naming the fix:

- `{issue}{desc}`, `{desc}{issue}`, `{type}{desc}` : two placeholders with no literal between them
- `{desc}-{desc}` : the same placeholder twice, since every occurrence expands to the same value

**Compiles, but drops a segment**, so `gwm doctor` warns and says which placeholder to add:

- `feat/#{issue}-{desc}` : hardcodes the type, so gitmoji selection and `[pr_template.by_type]` have none to read and `gwm commit-prefix` errors. This holds even when `feat` is the only configured branch type: the literal is text, and gwm declines to guess that it denotes a type
- `{type}/#1-{desc}` : freezes the issue number, so issue auto-linking from the branch name is inactive
- `{type}/#{issue}` : no `{desc}`, so `gwm pr` body placeholders and the TUI rename have no description
