...
1. **A pattern whose split can move.** Two placeholders written back to back leave the parser no boundary at all: `{issue}{desc}` writes `42123-x` from `42` and `123-x`, which reads back as `4212` and `3-x`, and `{desc}{issue}` is worse, because `a12` is what both `a` + `12` and `a1` + `2` produce, so no parser can be right. A non-empty separator is not automatically safe either. `{type}-{issue}9{desc}` writes `feat-42919x` from issue `42` and desc `19x`, and the greedy `\d+` slides right across the `9` to read issue `4291` and desc `x`.

   The condition is two-sided: the separator's first character has to be swallowable by the placeholder on its **left** *and* producible by the one on its **right**, so the displaced text can hand back a replacement separator. That is narrower than "the separator must sit outside the left placeholder's charset", which would reject a pattern that works: `-` after `{desc}` satisfies the first half and never the second, since an issue number cannot contain it, so `{desc}-{issue}` has exactly one valid split and stays legal. All of these are refused with a message naming the fix.
...
The recovery is an exact match, not a guess. A branch type is looked up in the repo's configured list, so `feature/{issue}-{desc}` recovers nothing: `feature` names a namespace, not a type. An issue number has to be all digits. A description is whatever `DESC_RE` accepts, which is the loosest of the three, so a leading namespace can be mistaken for one: `wt/{type}/#{issue}` reads `wt` as the description of every branch. `gwm doctor` reports that as a description that does not match what you asked for, which is how you find it. Write `{desc}` into the pattern if you want a real one.
