...
`{session}` is the detected session id. It is read out of the tool's own artefacts on disk, so it reaches the shell **quoted** and is substituted in a single pass, the same treatment [lifecycle hook placeholders](/configuration/gwm-toml#hooks) get. An empty string reads as unset and gives the default back, the convention `shell_cmd` / `editor_cmd` already use.
