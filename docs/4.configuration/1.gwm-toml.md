...
An unknown value is a **hard config error at load time**. Both `layout` and `dim_unfocused` are also exposed in the Settings panel under the **TUI** tab (`4`), where cycling the choice applies live.
