...
| Focus signal | header text + fill (`selection_bg` when focused, `section_bg` otherwise), and the inactive pane's body is dimmed | border colour |
...
An unknown value is a **hard config error at load time**.
