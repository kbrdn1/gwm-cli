...
# Drop the box rules around the panes and delimit sections with a filled
# one-line header instead. Off by default. See "compact layout" below.
compact = false
...
`compact` (issue [#545](https://github.com/kbrdn1/gwm-cli/issues/545)) drops the box rules around the worktrees pane and the sidebar sections, replacing each with a filled one-line header. A bordered section spends two rows and two columns on its frame plus a third row on its bottom-rule counter; a compact one spends a single row.

| | Bordered (default) | Compact |
|:--|:--|:--|
| Section frame | four rules, title in the top one | one filled header line |
| Title | ` Issue / PR [F] ` | ` ISSUE / PR [F] ` (same shape, uppercased) |
| Counter | bottom rule, right-aligned | right of the header line |
| Focus signal | border colour | header text **and** fill (`selection_bg` when focused, `section_bg` otherwise) |
| Worktrees pane height | its share of the stacked split | its row count, capped at that share |

The header fill is the `section_bg` [theme role](/tui/themes), an indexed colour rather than a translucent white so it stays readable on a terminal without truecolor. Each preset keeps it below its own `selection_bg` so the cursor remains the strongest background on screen.

Off by default: the mode changes what every screenshot in this documentation looks like. Overlays and modals keep their borders whatever the setting — a panel floating over content is where a rule earns its keep.
...
| `section_bg`   | compact-mode section header fill (`[tui] compact`)      | `236`         |
