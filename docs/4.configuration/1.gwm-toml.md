...
| Focus signal | header fill (a solid `focus` band when focused, `section_bg` otherwise) + a bold title | border colour |
...
An inactive header is the `section_bg` [theme role](/tui/themes), an indexed colour rather than a translucent white so it stays readable on a terminal without truecolor, with the title on it in `accent`. The focused one swaps the pair ([#605](https://github.com/kbrdn1/gwm-cli/issues/605)): a solid `focus` band carrying that section tone as its text, bold. Neither state uses `muted` — a pane's name is how you find the pane to `Tab` into, so it stays legible when the pane is inactive, and the signal you hunt for is the band, not a shade of grey.

On the solid band every span takes the band's pair, the ones that carry their own colour included: nothing readable survives an arbitrary colour on a saturated fill. The inactive band leaves them alone, so a filter `/` prompt still says what it says there.
