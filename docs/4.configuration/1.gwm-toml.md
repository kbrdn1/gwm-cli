...
| Focus signal | header fill (`selection_bg` when focused, `section_bg` otherwise) + a bold title | border colour |
...
The header *text* is the `accent` role in both states and does not track focus ([#605](https://github.com/kbrdn1/gwm-cli/issues/605)): a pane's name is how you find the pane to `Tab` into, so it stays legible when the pane is inactive. Focus adds weight to the whole header line, coloured spans included, on top of the fill.
