...
An unknown value is a **hard config error at load time**. `layout`, `dim_unfocused`, `status_one_line`, `note_vim`, `mux_open_in`, `mux_pane_direction` and `terminal_browser` are also exposed in the Settings panel under the **TUI** tab (`4`), where cycling the choice applies live.
