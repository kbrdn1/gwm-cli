...
description: gwm init --preset <stack> seeds an opinionated .gwm.toml for a known stack — the six built-ins (laravel, node/nuxt, rust, go, python-uv, generic), what each seeds, --list-presets and --show.
navigation:
  title: Config presets
---
...
|:------------|:---------------------|:-------------------------------------------------------------|:------------------------------------------------------------------------|
| `generic`   | —                    | —                                                            | — (the documented default, same as `gwm init`)                          |
...
Preset bodies are embedded in the binary. The five stack presets are kept in
...
its body *is* [`examples/gwm.toml.example`](https://github.com/kbrdn1/gwm-cli/blob/main/examples/gwm.toml.example),
