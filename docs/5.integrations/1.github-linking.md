...
The `source` field carries the link's provenance: `"branch-name"` (derived from the `<type>/#<N>-<slug>` convention, issues only), `"explicit"` (a recorded `gwm link`), `"detected"` (the new `LinkSource::Detected`, resolved live from `gh`), or `"none"`.
