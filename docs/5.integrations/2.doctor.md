...
- **It leaves the empty `[branch "…"]` header behind.** libgit2 removes the keys, not the section, so a branch that carried seven keys drops to one line rather than zero. On a repo grafted to 1434 lines, purging 869 keys brought the file to 561, not to 13.
