---
title: gwm doctor
description: Health checks with ✓ / ! / ✗ reporting, 0 / 1 / 2 exit codes for CI, and a --fix for the config a deleted branch left behind.
---
...
![Coloured gwm doctor output with ✓ sigils](./_assets/doctor.png)
...
### 7. no orphan worktree notes

Flags a note under `<main-checkout>/.git/gwm/notes/` whose branch no longer exists. Added by [#515](https://github.com/kbrdn1/gwm-cli/issues/515) with the notes themselves.

- **`✓`**: every stored note still has its branch
- **`!`**: at least one orphan, named, with the notes directory as the remediation

Warning and never Failed, and deliberately not wired into `gwm clean` or `gwm remove`: a note outliving its worktree is the point (the work has not landed yet), and it costs a few hundred bytes.

### 8. no orphan branch config

Reports `branch.<name>.gwm-*` keys whose branch no longer exists. Added by [#633](https://github.com/kbrdn1/gwm-cli/issues/633).

- **`✓`**: no gwm keys left behind by a deleted branch
- **`!`**: at least one, counted and named (past five branches, the list is elided), with `gwm doctor --fix` as the remediation

Unlike checks 5 and 6, this one is not cosmetic. `gwm list` opens one `Repository` per worktree, and libgit2 reparses `.git/config` on every open; `statuses()` and `branch.upstream()` then each take a config snapshot, which duplicates every entry. Both costs are linear in the file's size, so a listing pays for branches that died months ago. Measured with 8 worktrees held constant and the config size as the only variable: 45 ms at 13 lines, 184 ms at 1434.

`gwm create` writes about seven of these keys per branch and nothing removes them when the branch goes away, so the file only ever grows.

**A repo that follows this project's own "never delete the source branch" rule will find nothing here, and still pay the file's size on every listing.** Measured on gwm-cli itself: 917 `gwm-*` keys across 193 local branches, every one of them alive, so `--fix` purges zero. The keys accumulate where branches are deleted, which is most repos but not this one. That is why the listing was also made parallel: the purge is the lever for a repo that prunes its branches, and parallelism is the lever for one that does not.

The sweep matches the `branch.<name>.gwm-` **prefix**, not a list of known key names, so a key added in a later release is covered without anyone having to remember. Keys git itself writes (`remote`, `merge`, `rebase`, …) are never touched, on a live branch or a dead one.
...
The only check with an automatic remedy, and opt-in because it edits `.git/config`. A plain `gwm doctor` never writes. `--fix` runs before the report, so the checks printed under it reflect the repaired state.

Three things it deliberately does not do:

- **It works on the local config level only.** Deleting through the merged view would let git resolve the key to whichever file holds the highest-priority copy, up to `~/.gitconfig`.
- **It does not touch a file it does not own.** A key can come from a file pulled in by `include.path`: git resolves the deletion through the merged view and reports success, but only `.git/config` is ever rewritten, so the key survives. `--fix` reads the config back after writing and names the survivors rather than counting them as purged. The same line covers a config that could not be written at all (locked, read-only), and a purge that fails never suppresses the report: a diagnostic command that loses its diagnosis because its optional remedy failed is the wrong trade.
- **It leaves the empty `[branch "…"]` header behind.** libgit2 removes the keys, not the section, so a branch that carried seven keys drops to one line rather than zero. On a repo grafted to 1434 lines, purging 869 keys brought the file to 561 lines, not to 13. That figure is measured rather than `1434 - 869`: a multi-valued key spans several lines, and keys belonging to branches that are still alive are left alone.

### 9. base directory writable
...
### 11. `worktree.branch_pattern` round-trips through the parser
