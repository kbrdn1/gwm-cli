...
Only predicates the doctor can bound get evaluated: `cmd_exists:` (a `$PATH` lookup), `env_set:` / `env_eq:` (a read of the process environment), and `file_exists:` on a repo-relative path (one `stat`). A `.gwm.toml` has not been through the [trust gate](/configuration/trust-ledger), so a `glob_exists:` pattern out of it would choose its own root, and `glob_exists:/**/nope` is a whole-disk walk on a check meant to be instant. One such atom leaves the whole expression unevaluated, and the step stays probed. A `file_exists:` path is checked as resolved rather than merely spelled: a repo can commit a symlink, so `outside/etc/passwd` with `outside -> /` looks repo-relative and still leaves the repo once `exists()` follows it.
