...
`cargo binstall` resolves the archive (`gwm-v{version}-{target}.tar.gz`, `.zip` on Windows) straight from the matching GitHub Release and unpacks the binary — no Rust toolchain invocation and no libgit2 compile at install time, in contrast to `cargo install gwm` which builds the crate locally.
