...
> The daemon is **one per repo** — it answers for the repository it was launched in. Start it from inside the worktree set you want to watch (`cd` into any worktree of the repo, then `gwm daemon`).
...
zsh — right prompt (refreshed on each prompt; cheap because the daemon already holds the state — one socket round-trip, no git scan):
...
Without a running daemon, the same data is one `gwm` invocation away — `gwm list --format=json` and `gwm path <pattern> --format=json` emit the identical schemas — but the daemon avoids a process spawn (and a fresh git scan) per query.
