...
::: warning
The key is read from the repo's own `.gwm.toml`, which is a versioned file. A repository you do not trust can therefore point gwm at its own server. Treat cloning an untrusted repo as you would running its build scripts.
:::
