...
The TUI's static render labels and key hints still read "PR" everywhere. The forge exposes the right noun (`PR` / `MR`) and the CLI paths that build a message at runtime already use it; sweeping the remaining render strings is tracked separately.
