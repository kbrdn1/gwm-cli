...
2. **`$GITLAB_HOST` / `$GH_HOST` are pinned** when the origin is an `http(s)` URL, which states the web endpoint outright. An SSH remote does not — `https://<ssh-host>` is a guess, and forcing a guess over a working CLI config breaks setups that were fine — so SSH remotes rely on guard 1 alone.

`$GH_HOST` is pinned even for github.com: the child inherits gwm's environment, so an ambient `GH_HOST=github.acme.internal` would otherwise retarget a github.com repo, since the argv only carries `--repo owner/repo` and never a hostname.

### installs under a URL prefix

GitLab can be installed under a path (`https://example.com/gitlab`). From the remote alone that is indistinguishable from a project at `gitlab/group/proj` on example.com, so declare the instance root:

```toml
forge = "gitlab"
forge_host = "https://example.com/gitlab"
```

The declared root re-roots every generated URL, strips its prefix off the slug, and pins the CLI's host. It outranks anything inferred — including on an SSH remote, where you stating the answer beats a guess.
