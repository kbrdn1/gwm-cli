...
`gh` is the exception to guard 3: `--repo owner/repo` carries no hostname and `gh api repos/<slug>/…` bakes the slug into the request path, so neither can defer to the working directory. Its host is therefore pinned even for a guessed origin, unless that origin is github.com.
