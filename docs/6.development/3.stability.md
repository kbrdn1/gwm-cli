...
Three of these surfaces are *mechanically* frozen, and a rename fails CI before
...
- **Internal Rust API**: the `gwm-cli` crate publishes a `[lib]` target
  ...
  **no SemVer guarantee**. Do not `cargo add gwm-cli` to depend on `gwm::*`;
  ...
  library API is *disclaimed*, not gated with `cargo-semver-checks`, because owning
