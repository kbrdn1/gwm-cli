...
Both v0.10.0 pre-release trains (rc.1 and the rc.2 train above) are now on `dev`, queued for `v0.10.0-rc.2`. The previously-listed larger items — PTY overlay (#35), multi-repo workspace mode (#36), config presets (#37), and the JSON-RPC API + daemon (#38) — have all shipped in that train.

New direction — closing the GitHub loop (post-rc.2 product-gap review):

- [#308](https://github.com/kbrdn1/gwm-cli/issues/308) — `gwm review <PR#>`: materialise an existing PR / remote branch into a worktree. Today every worktree⇄GitHub feature is outbound-only; this adds the inbound half so you can check out a teammate's PR in isolation to review / test / fix it, turning the rc.2 CI indicator (#299) into a complete review workflow.
- [#309](https://github.com/kbrdn1/gwm-cli/issues/309) — a first consumer for `gwm daemon` (#38): a thin statusline one-liner over the `subscribe` stream plus an editor recipe, so the wire protocol is exercised by a real client.
