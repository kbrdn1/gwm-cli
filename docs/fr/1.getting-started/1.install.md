...
`gwm-cli-bin` est un paquet **binaire pré-compilé** : il télécharge la tarball linux-gnu `x86_64` ou `aarch64` depuis la Release GitHub correspondante, vérifie son `sha256`, puis installe le binaire `gwm`, les deux textes de licence et les complétions shell bash/zsh/fish. Aucune compilation, aucune toolchain Rust. Il déclare `provides`/`conflicts` sur `gwm-cli` **et** `gwm` (il possède `/usr/bin/gwm`), donc il ne cohabite pas avec une build source du même outil.
...
Le flake sert `x86_64-linux`, `aarch64-linux` et `aarch64-darwin`. macOS Intel (`x86_64-darwin`) n'en fait pas partie : le nixpkgs épinglé par ce flake a abandonné cette plateforme, utilisez donc l'archive pré-compilée, `cargo binstall` ou `cargo install` sur ce système.
