...
description: Pas à pas de `gwm create` — nommage de branche, organisation des chemins et pipeline de bootstrap.
---
...
- Une entrée de config git `branch.feat/#42-user-authentication.gwm-base`, qui ancre la [chaîne de résolution de la base de review](/fr/tui/launchers#base-resolution) sur le tronc depuis lequel vous avez branché

La forme de `gwm create` est `gwm create <type> <issue> <description>`. Les valeurs de `<type>` prises en charge sont documentées dans [Configuration → schéma `.gwm.toml`](/fr/configuration/gwm-toml#supported-branch-types) — `feat`, `fix`, `hotfix`, `docs`, `test`, `refactor`, `chore`, `perf`, `ci`, `build` par défaut.

## conventions de branche et de chemin

Les valeurs par défaut — et la manière de les surcharger — sont pilotées par TOML :
...
Placeholders disponibles : `{home}`, `{repo}`, `{type}`, `{issue}`, `{desc}`. Le tilde (`~/…`) est également développé. Surchargez ce que vous voulez dans votre `.gwm.toml` pour correspondre à la convention de branche de votre équipe — voir [Configuration → schéma `.gwm.toml`](/fr/configuration/gwm-toml).
...
Si un `.gwm.toml` est présent à la racine du dépôt, `gwm create` exécute son pipeline de bootstrap immédiatement après le `git worktree add` :
...
5. **Commandes** — les hooks shell `[[bootstrap.command]]` s'exécutent en dernier, conditionnés par des [prédicats `when:`](/fr/configuration/when-predicates) (`file_exists:`, `cmd_exists:`, `env_set:`, …).

Le rapport de bootstrap s'imprime inline avec les sigles `✓` (succès), `!` (avertissement, non bloquant) ou `✗` (échec, bloque le worktree) par étape — même convention que `gwm doctor`. Un `✗` annule et le worktree est rollback.

Sautez entièrement le bootstrap avec `--no-bootstrap` :
...
- `branch.<name>.gwm-base` enregistre le tronc depuis lequel cette branche a été coupée. Il est utilisé par le [launcher de review](/fr/tui/launchers#base-resolution) pour calculer `git diff base..head` même après que l'upstream de la branche a disparu.
- Si vous avez utilisé la convention d'auto-link (`<type>/#<N>-<slug>`), le lien vers l'issue GitHub est dérivé à la volée — aucune config supplémentaire nécessaire. Voir [Intégrations → liaison issue / PR GitHub](/fr/integrations/github-linking).
