...
**`M` est la voie.** Lire la souris et laisser le terminal sélectionner du
texte sont mutuellement exclusifs : tant que gwm lit, un glissement lui
appartient et le terminal n'exécute pas sa propre sélection. `M` coupe le
rapport jusqu'à la prochaine pression, l'en-tête porte une pastille
` mouse off · M ` tant qu'il est coupé pour que le mode ne soit jamais
silencieux, et le réglage survit à un passage par lazygit, un `exec` ou un
lanceur de review — revenir d'un enfant plein écran ne rend pas la souris à gwm
en douce.

Si l'échange est dans le mauvais sens pour vous, `[tui] mouse = false` dans
`.gwm.toml` démarre chaque session avec la souris relâchée, et `M` l'active
quand vous voulez cliquer. Voir
[Configuration → `mouse`](/fr/configuration/gwm-toml#mouse).

gwm garde sa part de l'échange aussi petite que possible :
il ne demande que le **suivi des pressions** (`1000`), jamais le rapport de
glissement et de mouvement (`1002` / `1003`) que la plupart des TUI activent
avec. Rien ici ne lit un glissement, donc rien ici n'en demande.

`Maj` + glisser est la convention pour atteindre la sélection du terminal
par-dessus une TUI, et elle vaut le coup d'être essayée — mais c'est le
terminal qui l'honore, pas l'application, et tous ne l'honorent pas. **Mesuré
sur Ghostty 1.3.1 sous herdr : non.** Les terminaux qui l'exposent l'appellent
`mouse-shift-capture` (Ghostty) ou « Terminal may report mouse events »
(iTerm2).
