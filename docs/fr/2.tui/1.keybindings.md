...
| `M`         | relâche / reprend la souris (`toggle_mouse`), voir [souris](#souris-m)                            |
| `gg`        | sauter au premier worktree (`top`)                                                                |
| `G` / `End` | sauter au dernier worktree (`bottom`)                                                             |
| `←` / `→`   | replier / déplier le groupe de dépôt de la ligne survolée (`collapse_group` / `expand_group`), mode workspace uniquement, voir [groupes de repos](#groupes-de-repos-mode-workspace) |
...
La TUI lit les événements souris ([#624](https://github.com/kbrdn1/gwm-cli/issues/624)).
Rien à activer : tout terminal qui rapporte la souris fonctionne.

| Geste                                | Effet                                                                     |
|:-------------------------------------|:--------------------------------------------------------------------------|
| clic sur une ligne de worktree       | la sélectionne et donne le focus au panneau des worktrees                 |
| clic sur un panneau                  | lui donne le focus, comme `Tab` / `1` / `2`                                |
| clic sur une ligne d'une modale      | déplace le curseur de cette liste                                          |
| clic sur le titre d'une section de la sidebar | ouvre cette section en plein écran (`Issue / PR` → `I`, `Agents` → `a`, `Working Tree` → `W`, `Recent Commits` → `c`) |
| clic sur un champ du formulaire de création / renommage | lui donne le focus                               |
| clic sur les `‹` / `›` du sélecteur de type | recule / avance d'un cran                                          |
| clic sur `Confirm` / `Cancel`        | met le focus sur ce bouton et l'active                                     |
| clic sur un onglet des Paramètres    | bascule dessus                                                             |
| clic sur `▤` / `⚙` dans l'en-tête    | ouvre le panneau Command Logs / Paramètres, comme `3` / `4`                |
| clic sur le `✕` d'une modale         | la ferme, comme `Esc`                                                      |
| clic sur les compteurs Working Tree  | ouvre la liste Working Tree plein écran, comme `W`                         |
| molette                              | fait défiler ou déplace la sélection de ce qui est **sous le pointeur**    |

La molette suit le pointeur plutôt que le focus : pointer un panneau, c'est
la façon dont un pointeur dit de quel panneau il parle. Le focus reste ce que
`Tab` et les chiffres définissent, et `j` / `k` le suivent toujours.

### récupérer la sélection de texte du terminal

Lire la souris et laisser le terminal faire sa propre sélection de texte sont
mutuellement exclusifs : tant que gwm lit, un glissement lui appartient. Il y a
trois façons d'avoir les deux, dans l'ordre où vous les voulez probablement.

**1. `Maj` + glisser.** Le juste milieu, et celui qui ne renonce à rien : le
geste atteint la sélection du terminal par-dessus la TUI, et tous les clics
continuent de marcher. C'est le terminal qui l'honore, pas l'application, donc
c'est son réglage qui tranche — et un réglage qui le coupe est facile à avoir
sans le vouloir. Vérifié sur Ghostty 1.3.1 sous herdr :
`mouse-shift-capture = false` (le défaut) sélectionne le texte, `always` envoie
le geste à gwm et rien n'est sélectionné. iTerm2 nomme le même bouton
« Terminal may report mouse events ».

**2. `M`, pour le moment présent.** Coupe le rapport jusqu'à la prochaine
pression. L'en-tête porte une pastille ` mouse off · M ` tant que c'est coupé,
donc le mode n'est jamais silencieux, et le réglage survit à un passage par
lazygit, un `exec` ou un lanceur de review — revenir d'un enfant plein écran ne
rend pas la souris à gwm en douce.

**3. `[tui] mouse = false`, pour de bon.** Démarre chaque session avec la
souris relâchée, et `M` l'active quand vous voulez cliquer. Le bon choix si
vous copiez des chemins à la souris plus souvent que vous ne cliquez sur des
lignes. Voir [Configuration → `mouse`](/fr/configuration/gwm-toml#mouse).

gwm garde sa propre part de l'échange aussi petite que possible : il ne demande
que le **suivi des pressions** (`1000`), jamais le rapport de glissement et de
mouvement (`1002` / `1003`) que la plupart des TUI activent avec. Rien ici ne
lit un glissement, donc rien ici n'en demande.

**Sous un multiplexeur, c'est le multiplexeur qui arbitre.** tmux, zellij et
herdr ont chacun leur propre gestion de la souris, et ce qui parvient à gwm —
comme ce que fait un glissement — est leur décision avant d'être celle du
terminal. Si un geste ne correspond pas à cette page, ce réglage est le premier
endroit à regarder (`set -g mouse` sous tmux). Refaire le même geste hors du
multiplexeur est ce qui départage les deux.

La copie directe contourne toute la question : `Y` copie le chemin du worktree,
`y` le nom de branche, `w` le nom du worktree, et elles passent par OSC52 en
SSH.
...
`Space` était le cycle de disposition de la barre latérale avant l'[#484](https://github.com/kbrdn1/gwm-cli/issues/484) ; ce verbe vit maintenant sur `z`. Les deux valeurs par défaut sont à une ligne de `[tui.keys]` (`cycle_sidebar_layout = ["Space"]`, `toggle_select = ["z"]`). Si votre `.gwm.toml` lie un chord qui *commence* par `z` (par ex. `top = ["z z"]`), c'est désormais un conflit de préfixe avec une valeur par défaut livrée, refusé au chargement : remappez-le, ou déplacez `cycle_sidebar_layout` ailleurs.

## groupes de repos (mode workspace)

En [mode workspace](/fr/cli/reference#mode-workspace-global---workspace-issue-36),
la table liste les worktrees de tous les dépôts en une seule liste, le worktree
principal de chaque dépôt en tête. Depuis l'[#680](https://github.com/kbrdn1/gwm-cli/issues/680),
chaque dépôt est un accordéon dont l'en-tête est cette ligne principale : `←`
replie le groupe où se trouve le curseur sur son en-tête, `→` le déplie.
L'en-tête porte un chevron dans sa cellule d'âge (`▾` ouvert, `▸` replié ; la
case est libre car une branche tronc n'a pas d'âge), et un en-tête replié se
peint dans le rôle `muted`, pour qu'un dépôt replié se lise comme tel d'un coup
d'œil. Un dépôt à un seul worktree n'a rien à replier : pas de chevron, et `←`
n'y enregistre aucun pli.

Replier déplace le curseur sur l'en-tête, puisque la ligne où il se trouvait est
en général l'une de celles que le pli cache, et retire toute marque posée sur
une ligne cachée : une marque hors écran est une ligne que `d` supprimerait
sans la montrer. Un filtre `/` actif l'emporte sur le pli (la requête cherche
dans les noms de worktrees, elle peut donc retenir une ligne liée tout en
filtrant son en-tête), masque les chevrons tant qu'il dure, et rend `←` / `→`
inertes : aucun pli n'est enregistré sous une requête, et effacer la requête
retire les marques que le pli de retour cache. L'état de pli ne vit que le
temps de la session : rien ne le persiste, et chaque groupe démarre ouvert. Les deux touches sont sans effet en mode mono-dépôt, et se remappent en
`collapse_group` / `expand_group` sous `[tui.keys]`.
...
`N` n'était lié à rien avant l'[#515](https://github.com/kbrdn1/gwm-cli/issues/515). Si votre `.gwm.toml` lie un chord qui *commence* par `N` (par ex. `top = ["N x"]`), c'est désormais un conflit de préfixe avec une valeur par défaut livrée, refusé au chargement : remappez-le, ou déplacez `edit_note` ailleurs.
...
seule (config résolue), chacun précédé de son glyphe. `L` bascule la couche
d'édition entre le `.gwm.toml` du projet et la config globale utilisateur ; le
sélecteur de couche décide dans quel fichier une édition est écrite.

Depuis [#623](https://github.com/kbrdn1/gwm-cli/issues/623), chaque onglet se
lit en deux colonnes : ce qu'est une ligne à gauche, ce à quoi elle est réglée à
droite, aligné sur le bord du panneau lui-même, pour occuper sa largeur plutôt
que de la laisser vide à côté d'un bloc dimensionné au contenu. Sur un panneau
trop étroit pour les deux, l'écart retombe à un minimum et la ligne défile
horizontalement, comme avant.

Sur les onglets éditables, la forme d'une valeur dit de quel type elle est :

| La valeur se lit | Type | Touches |
|:--|:--|:--|
| `‹ valeur ›` | un choix, et les marqueurs annoncent qu'il cycle | `←` / `→`, ou `Space` / `Enter` |
| `[✓]` / `[ ]` | un booléen, c'est-à-dire un cycle à deux valeurs | `←` / `→`, ou `Space` / `Enter` |
| du texte ou un nombre | saisi, pas cyclé | `Space` / `Enter` ouvre le tampon d'édition |
| `(unset)` | un champ texte optionnel que personne n'a défini | `Space` / `Enter` |

L'onglet `TUI`, le plus long, groupe ses seize lignes sous des règles
nommées : **Appearance**, **Sidebar**, **Editing**, **Open**, **Multiplexer**,
**Browser**, **Timing**. Les onglets `Keys` et `All` portent les mêmes règles
sur leurs portées `[global]` / `[modal.<contexte>]` et leurs sections
`[table]`, et placent leur binding et leur valeur résolue dans la même colonne
de droite.
...
| `←` / `→`         | ajuster le choix sélectionné d'une valeur en arrière / en avant (pan sur l'onglet `All`, sans effet sur `Keys`) |
...
Le titre porte le nombre de fichiers modifiés, `Working Tree (27)` (issue
[#622](https://github.com/kbrdn1/gwm-cli/issues/622)). Il compte des fichiers,
pas des lignes, et s'accorde donc avec le pied : les lignes de dossier, la
mention de débordement `… N more` et la sentinelle `✓ clean` sont des lignes
sans être des fichiers modifiés. Il est omis tant que la liste est en cours de
lecture, un `(0)` y serait une affirmation plutôt qu'un décompte.
...
Plus à droite encore, dans sa propre colonne, la lettre de statut `M` / `A` /
`D` / `?` (issue [#622](https://github.com/kbrdn1/gwm-cli/issues/622)).

Les deux colonnes cèdent dans un ordre fixe. La lettre est réservée en
premier : un terminal qui rétrécit lâche les compteurs `+120 -34` et garde la
lettre ; assez étroit, les deux sautent, mais jamais le nom de fichier.
`+120 -34` dit de combien un fichier a changé, la lettre dit ce qui lui est
arrivé, et une ligne qui ne dit plus ce qu'elle est a perdu son sujet, pas un
détail.
...
session ouvre la surcouche avec une ligne explicite *no agent session found*
...
La surcouche se lit en deux colonnes depuis
[#623](https://github.com/kbrdn1/gwm-cli/issues/623) : la description à gauche,
l'accord qui lui correspond aligné à droite sur le bord de la surcouche, sous
les mêmes règles de section nommées que le panneau Paramètres.

Quand les deux colonnes se disputent une surcouche étroite, c'est l'accord qui
gagne : une description qui ne tient pas est tronquée par une ellipse, parce
qu'un mémo de raccourcis qui n'affiche aucun raccourci n'en est pas un. Les
descriptions ont donc été raccourcies : à 80 colonnes, le terminal le plus
étroit sur lequel il sert, rien n'est tronqué, et un test l'épingle : un
nouveau binding à description longue échoue plutôt que de raccourcir en silence
