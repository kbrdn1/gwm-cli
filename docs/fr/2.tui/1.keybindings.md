...
Tant que la souris est rapportée, un cliquer-glisser appartient à gwm et non à
la sélection propre du terminal : le glissement habituel pour copier un chemin
ne sélectionne rien. Deux issues :

- **`Maj` + glisser.** La plupart des terminaux (iTerm2, Ghostty, kitty,
  WezTerm, Alacritty, GNOME Terminal, Windows Terminal, Terminal.app)
  court-circuitent le rapport souris tant que `Maj` est maintenue et font leur
  propre sélection. C'est une convention, pas une garantie : le modificateur
  peut différer, ou manquer, sur un terminal qui ne la suit pas.
- **`M`.** Coupe le rapport souris jusqu'à la prochaine pression. La barre de
  statut le dit, et le réglage survit à un passage par lazygit, un `exec` ou un
  lanceur de review — revenir d'un enfant plein écran ne rend pas la souris à
  gwm en douce.

La copie directe reste la voie la plus courte : `Y` copie le chemin du
worktree, `y` le nom de branche, `w` le nom du worktree.
