...
Tant que la souris est rapportée, un clic appartient à gwm et non à la
sélection propre du terminal : le cliquer-glisser pour copier un chemin peut ne
rien sélectionner.

**`M` est la réponse qui marche toujours.** Elle coupe le rapport souris
jusqu'à la prochaine pression ; la barre de statut le dit, et le réglage survit
à un passage par lazygit, un `exec` ou un lanceur de review — revenir d'un
enfant plein écran ne rend pas la souris à gwm en douce.

`Maj` + glisser vaut le coup d'être essayé d'abord, et gwm fait ce qu'il peut
pour le laisser passer : il ne demande que les deux modes souris qu'il lit
réellement (pression et molette) et coupe le rapport de glissement / mouvement
qu'`EnableMouseCapture` activerait sinon — ce sont ces modes-là qu'un terminal
lit pour décider que « l'application veut toute la souris ». Que `Maj`
court-circuite ensuite le rapport reste la décision du terminal, et tous ne la
prennent pas : **Ghostty 1.3.1 ne le fait pas** (mesuré). Vérifiez le réglage
de votre terminal avant de présumer : Ghostty a `mouse-shift-capture`, iTerm2 a
...
terminal. Si le clic et le glissement ne correspondent pas à ce que dit cette
page, vérifiez d'abord le réglage souris du multiplexeur (`set -g mouse` sous
tmux).
