...
par session, la plus récente d'abord, avec le nom de l'agent, sa fraîcheur
(**active** = activité d'artefact dans les 5 dernières minutes, **idle**
sinon) et une heure de dernière activité lisible. La détection lit les
artefacts de session sur disque de chaque agent (Claude Code, Codex,
opencode, Mistral Vibe) — aucun scan de processus, donc un comportement
identique sur Linux, macOS et Windows. Un worktree sans session ouvre la
surcouche avec une ligne explicite *no agent session found* plutôt qu'une
modale vide.
