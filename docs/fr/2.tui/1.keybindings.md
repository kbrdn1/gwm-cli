...
> [Configuration → `[tui.keys]`](/fr/configuration/gwm-toml#tui-keys).
...
| `F`         | rafraîchir le statut de l'issue / PR GitHub (fetch `gh` synchrone)                                |
...
| `s`         | basculer le mode Détails de la barre latérale — `commits` ↔ `stashes` (action remappable `toggle_sidebar_mode`) — voir [mode stashes](/fr/tui/sidebar#stashes-mode) |
...
| `:`         | ouvrir la [palette de commandes](/fr/tui/keymap-and-palette#command-palette) (action remappable `command_palette`) |
