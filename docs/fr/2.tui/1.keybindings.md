...
| `N`         | ouvrir la note du worktree sélectionné dans `$EDITOR` (`edit_note`), voir [notes](#notes-n)      |
...
`N` ouvre la note du worktree sélectionné dans `$EDITOR`. gwm se met en veille, l'éditeur prend le terminal, et le TUI revient à la sortie de l'éditeur : c'est le même relais que `o` en [`mode = "editor"`](/fr/tui/open-dispatch), soit `editor_cmd` dans `.gwm.toml`, puis `$EDITOR`, puis `vi`.
...
moins les blocs propres aux PR. Rien n'est récupéré en plus de ce que le
refresh de statut demande déjà, donc la vue est aussi fraîche que le
dernier `f`. Si aucun des deux côtés n'a été récupéré, rien ne s'ouvre et la
barre de statut indique quoi faire.
...
récupérées : ces sections sont donc absentes plutôt qu'affichées vides.
