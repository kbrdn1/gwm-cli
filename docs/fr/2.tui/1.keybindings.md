...
running) et le nom du check. S'ouvre depuis n'importe où dans la vue liste
avec `C`, ou avec `c` quand le pane status a le focus (le même dispatch
contextuel qui transforme `j` / `k` en défilement de la barre latérale).
Sans PR liée ou avec un rollup vide, rien ne s'ouvre et la barre de statut
explique pourquoi.
...
| `f`            | filtrer la liste (`filter`) — requête sous-chaîne en direct, Enter ouvre le surligné |
| `Esc` / `q`    | fermer (`close`)                                                          |
