...
| `f`            | filtrer la liste (`filter`) — requête sous-chaîne en direct, Enter ouvre le surligné. Pendant la saisie, les touches imprimables alimentent la requête (convention palette, comme l'invite d'attache) ; les verbes remappables s'appliquent en mode liste |
| `Esc` / `q`    | fermer (`close`)                                                          |
