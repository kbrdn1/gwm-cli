...
Une ligne par session d'agent IA attachée au worktree sélectionné (`claude`
/ `codex` / `opencode` / `vibe`), colorée par fraîcheur (**active** /
**idle**), avec la dernière activité lisible et le nom de la session (son
premier prompt, ou le titre Vibe — l'id complet sinon). Plafonné aux trois
sessions les plus récentes avec une ligne `+N more` au-delà ; appuyez sur
`a` pour la liste interactive complète dans la
[surcouche des sessions d'agents](/fr/tui/keybindings#surcouche-des-sessions-dagents-a).
Le bloc disparaît entièrement quand aucune session n'est détectée. Ajouté
par [#408](https://github.com/kbrdn1/gwm-cli/issues/408).
