...
- **`Agent`** — la session d'agent IA la plus récemment active attachée à ce worktree (`claude` / `codex` / `opencode` / `vibe`), avec sa fraîcheur (**active** / **idle**) et un compteur `+N` quand d'autres sessions correspondent. Masquée quand aucune session n'est détectée. Appuyez sur `a` pour la liste complète dans la [surcouche des sessions d'agents](/fr/tui/keybindings#surcouche-des-sessions-dagents-a). Ajouté par [#408](https://github.com/kbrdn1/gwm-cli/issues/408).
