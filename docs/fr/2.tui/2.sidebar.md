...
- La **lettre de statut** `M` / `A` / `D` / `?` occupe **sa propre colonne
  ...
  Elle garde la couleur de sa catégorie. La colonne se place un espace après
  la ligne la plus longue et non contre la bordure du panneau : la sidebar
  est empilée par défaut, donc sur un terminal large une lettre collée au
  bord atterrirait à cent cellules de son nom.
