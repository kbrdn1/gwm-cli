...
- `Space` (`cycle_sidebar_layout`) fait défiler la disposition `auto → side-by-side → stacked → auto`. `auto` est la valeur par défaut pilotée par la largeur ci-dessus ; les deux autres figent l'orientation quelle que soit la largeur.
