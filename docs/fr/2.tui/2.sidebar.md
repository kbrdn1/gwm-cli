...
Les sections à hauteur variable — Agents, `Working Tree` et `Recent Commits` — se partagent la colonne de façon responsive : quand tout tient, chacune garde sa hauteur naturelle et `Recent Commits` absorbe le surplus ; quand le terminal est court, chaque section visible est garantie à **5 lignes minimum** (bordure + 3 lignes de contenu) et la hauteur restante est répartie proportionnellement à la taille du contenu. Le contenu qui déborde reste atteignable via les défilements de section (`j` / `k` pour `Recent Commits`, `J` / `K` pour `Working Tree`). Une section vide se replie toujours entièrement (arbre propre, aucune session d'agent).
