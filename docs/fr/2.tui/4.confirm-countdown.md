...
description: Le compte à rebours de sécurité qui protège la suppression de branche quand `p` est armé.
---

# Compte à rebours de la surcouche de confirmation

Ajouté par [#30](https://github.com/kbrdn1/gwm-cli/issues/30).

La surcouche de confirmation `d` a deux modes, choisis automatiquement selon que `p` (basculer « supprimer la branche au retrait ») a été pressé plus tôt dans la session.
...
Quand `p` est armé, la surcouche affiche la branche sur le point de disparaître ainsi qu'une étape `arm` en deux temps :
...
```toml
[tui]
# 3s par défaut. Mettre à 0 pour désactiver le compte à rebours (modal classique même quand p est armé).
...
- [Raccourcis clavier → surcouche de confirmation de suppression](/fr/tui/keybindings#confirm-delete-overlay) — les touches que cette page décrit
