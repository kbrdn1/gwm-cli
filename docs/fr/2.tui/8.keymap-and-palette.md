...
Vous pouvez éditer chacune d'elles — globales *et* modales — sans toucher `.gwm.toml` à la main, depuis l'**onglet Keys** du [panneau Paramètres](/fr/tui/keybindings#panneau-paramètres-4) (`4`) : sélectionnez un binding, appuyez sur la touche d'activation et capturez une nouvelle touche en direct ; la capture écrit le tableau TOML dans la couche ciblée, le valide et recharge le keymap sur-le-champ.
...
`open_in` vaut `"pty"` par défaut (une [surcouche PTY](/fr/tui/launchers) embarquée, la TUI se suspend jusqu'à la sortie) ; mettez `"mux_pane"` pour lancer la commande dans un nouveau panneau tmux / zellij à la place. Les bindings `h` / `H` sont eux-mêmes remappables comme les actions `macro_one` / `macro_two` dans `[tui.keys]`.
