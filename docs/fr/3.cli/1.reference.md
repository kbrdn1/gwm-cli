...
```bash
gwm agents                       # sessions par worktree : agent, fraîcheur, id
...
gwm agents detach feat-42        # retour à l'auto-détection pure
```
...
worktree précis quel que soit son point de départ. Une épingle par worktree
(un nouvel attach la remplace), stockée dans la config de branche git
(`gwm-agent-pin`), jamais commitée. Attacher un id que la détection ne
...
HEAD détachée ne peut pas porter d'épingle.
