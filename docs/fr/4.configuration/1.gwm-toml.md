...
Une valeur inconnue est une **erreur de config au chargement**. `layout`, `dim_unfocused`, `status_one_line`, `note_vim`, `mux_open_in`, `mux_pane_direction` et `terminal_browser` sont aussi exposés dans le panneau Settings sous l'onglet **TUI** (`4`), où faire défiler le choix s'applique à chaud.
