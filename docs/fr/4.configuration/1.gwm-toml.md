...
Le formulaire de renommage de la TUI affiche un segment figé, et sa modifiabilité dépend de l'endroit où la nouvelle valeur pourrait aller. `worktree.path_pattern` est interrogé lui aussi : quand il écrit le segment, le modifier renomme le répertoire du worktree et laisse la branche tranquille, ce qui est un vrai renommage et reste donc autorisé, et la preview du modal l'annonce en affichant la branche inchangée. Le refus ne tombe que si aucun des deux motifs n'écrit le segment, puisque la soumission reconstruirait la même branche et le même répertoire. Les segments que `branch_pattern` écrit restent toujours éditables.
