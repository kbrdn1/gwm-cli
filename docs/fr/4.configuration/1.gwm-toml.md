...
**clean — le `dirs` d'un profil est un jeu complet qui remplace les intégrés.** `[clean.profiles.<nom>].dirs` ne **s'ajoute pas** aux `target`/`node_modules`/`dist`/`build` intégrés — il les **remplace** intégralement. Chaque entrée doit être un chemin **relatif au worktree** : un chemin absolu, une remontée `..` ou une chaîne vide est rejetée (sortie 1), pour qu'une coquille ne puisse pas faire parcourir l'extérieur du worktree même en dry-run. La barrière de sûreté (git-ignored + aucun fichier suivi + skip des symlinks) s'applique toujours à chaque répertoire du jeu.
