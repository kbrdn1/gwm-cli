...
`{session}` est l'id de session détecté. Il est lu dans les artefacts du tiers sur le disque, donc il arrive au shell **quoté** et il est substitué en une seule passe, le même traitement que les [placeholders des hooks de cycle de vie](/fr/configuration/gwm-toml#hooks). Une chaîne vide vaut « non défini » et redonne la valeur par défaut, la convention que `shell_cmd` / `editor_cmd` utilisent déjà.
