...
Deux choses qu'un parseur dérivé ne peut toujours pas récupérer, et que `gwm doctor` / `gwm config validate` nomment toutes les deux :

1. **Un motif impossible à compiler.** Deux placeholders écrits côte à côte ne laissent aucune frontière où découper. `{issue}{desc}` écrit `42123-x` à partir de `42` et `123-x`, qui se relit `4212` et `3-x` ; `{desc}{issue}` est pire, puisque `a12` est ce que produisent à la fois `a` + `12` et `a1` + `2` : aucun parseur ne peut avoir raison. Les deux sont refusés avec un message qui nomme le correctif. La règle porte uniquement sur l'*adjacence* : un séparateur pris dans le jeu de caractères du placeholder de gauche ne pose aucun problème, donc `{desc}-{issue}` fait l'aller-retour (un numéro d'issue ne peut pas contenir le `-`, il n'existe donc qu'un seul découpage valide).
2. **Un segment que le motif n'écrit jamais.** `{type}/#1-{desc}` fige le numéro d'issue, `feat/#{issue}-{desc}` code le type en dur. Les deux créent très bien des worktrees, et dans les deux cas gwm n'a rien où lire ce segment. L'avertissement indique quel placeholder ajouter.
...
| `{type}-{issue}-{desc}` | sans slash, et non ambigu : `-` n'est ni dans la liste des types ni dans `\d+` |
...
**Compile, mais perd un segment**, donc `gwm doctor` avertit et indique quel placeholder ajouter :

- `feat/#{issue}-{desc}` : code le type en dur, donc la sélection du gitmoji et `[pr_template.by_type]` n'ont rien à lire et `gwm commit-prefix` échoue. C'est vrai même quand `feat` est le seul type de branche configuré : le littéral est du texte, et gwm refuse de deviner qu'il désigne un type
- `{type}/#1-{desc}` : fige le numéro d'issue, donc l'auto-liaison depuis le nom de branche est inactive
- `{type}/#{issue}` : pas de `{desc}`, donc les placeholders de corps de `gwm pr` et le renommage TUI n'ont pas de description
