...
**clean — le `dirs` d'un profil est un jeu complet qui remplace les intégrés.** `[clean.profiles.<nom>].dirs` ne **s'ajoute pas** aux `target`/`node_modules`/`dist`/`build` intégrés — il les **remplace** intégralement. La barrière de sûreté (git-ignored + aucun fichier suivi + skip des symlinks) s'applique toujours à chaque répertoire du jeu.
