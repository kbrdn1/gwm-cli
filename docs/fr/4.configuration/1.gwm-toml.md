...
| `cycle_sidebar_layout`    | `V`                 | faire cycler le layout (`auto` → côte-à-côte → empilé → auto) |
| `toggle_sidebar_position` | `H`                 | basculer la sidebar gauche ↔ droite                   |
| `focus_swap`              | `Tab`               | échanger le focus entre la table et la sidebar        |
| `filter`                  | `/`                 | ouvrir la barre de filtre fuzzy                       |
| `refresh`                 | `f`, `r`            | rafraîchir la liste des worktrees                     |
...
| `delete_branch`           | `p`                 | armer la suppression de branche au remove             |
| `git_tui`                 | `l`                 | lancer la git TUI (`[git_tui]`)                       |
| `review`                  | `R`                 | exécuter l'outil de review (`[review]`)               |
| `yank`                    | `y`                 | copier le chemin du worktree                          |
| `open`                    | `o`                 | dispatch d'ouverture (`[tui.open]`)                   |
| `open_menu`               | `O`                 | ouvrir le menu de mode d'ouverture                    |
| `link`                    | `L`                 | ouvrir le prompt de lien issue/PR                     |
...
- Les clés TOML inconnues sont silencieusement ignorées (compatible en avant avec de nouveaux champs).
