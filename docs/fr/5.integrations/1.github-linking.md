...
Chaque worktree peut être lié à une issue GitHub et / ou à une pull request. La liaison alimente le bloc `Issue / PR` en temps réel dans la [barre latérale du TUI](/fr/tui/sidebar#issue--pr-block) et apparaît dans `gwm status` pour le scripting.
...
Les numéros de PR n'ont pas de convention de nom de branche, ils sont donc détectés autrement : [#181](https://github.com/kbrdn1/gwm-cli/issues/181) a ajouté la **détection automatique éphémère des PR**. Lorsqu'une branche n'a pas de liaison PR explicite, gwm demande à GitHub quelle PR a été ouverte depuis elle (`gh pr list --head <branch> --state all`) et l'affiche marquée `detected` :
...
Le champ `source` porte la provenance de la liaison : `"branch-name"` (déduit de la convention `<type>/#<N>-<slug>`, issues seulement), `"explicit"` (un `gwm link` enregistré), `"detected"` (le nouveau `LinkSource::Detected`, résolu en temps réel depuis `gh`), ou `"none"`.

La détection n'est **jamais persistée** — elle est résolue en temps réel à chaque lecture, elle est donc toujours fraîche et un `gwm link pr` explicite l'emporte toujours. Comme chaque résolution est un appel `gh`, la détection ne s'exécute que là où elle est peu coûteuse ou demandée :

- **`gwm status`** — toujours (un seul worktree).
...
- **`gwm list --detect-pr`** — drapeau opt-in ; ajoute une colonne `PR` au prix d'un appel `gh` par worktree. Le `gwm list` simple reste sans réseau.
...
> Le raccourci `F` était `R` avant la v0.6 — réassigné par [#75](https://github.com/kbrdn1/gwm-cli/issues/75) lorsque `R` a été réservé pour le [lanceur de review](/fr/tui/launchers). Voir [Raccourcis → Résumé du réassignement v0.6](/fr/tui/keybindings#v06-rebind-summary).
...
- [TUI → Barre latérale](/fr/tui/sidebar#issue--pr-block) — où le bloc en temps réel s'affiche
- [TUI → Raccourcis](/fr/tui/keybindings#issue--pr-link-prompt-l) — les overlays `O` / `L` / `F`
