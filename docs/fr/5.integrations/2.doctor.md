...
Seuls les prédicats que le doctor sait borner sont évalués : `cmd_exists:` (une recherche dans le `$PATH`), `env_set:` / `env_eq:` (une lecture de l'environnement du processus) et `file_exists:` sur un chemin relatif au dépôt (un seul `stat`). Un `.gwm.toml` n'est pas passé par le [garde-fou de confiance](/fr/configuration/trust-ledger), donc un motif `glob_exists:` venu de là choisirait sa propre racine, et `glob_exists:/**/nope` est un parcours de tout le disque sur une vérification censée être instantanée. Un seul atome de ce genre laisse l'expression entière non évaluée, et l'étape reste sondée. Un chemin `file_exists:` est vérifié une fois résolu, pas seulement sur son écriture : un dépôt peut versionner un lien symbolique, donc `outside/etc/passwd` avec `outside -> /` a l'air relatif au dépôt et en sort pourtant dès que `exists()` le suit.
