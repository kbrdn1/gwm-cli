---
title: gwm doctor
description: Vérifications de santé avec un rapport ✓ / ! / ✗, des codes de sortie 0 / 1 / 2 pour la CI, et un --fix pour la config qu'une branche supprimée a laissée derrière elle.
---
...
![Sortie colorée de gwm doctor avec les sigles ✓](../../5.integrations/_assets/doctor.png)
...
✓ no orphan worktree notes
    2 note(s) stored, every branch still exists
✓ no orphan branch config
    no gwm keys left behind by a deleted branch
✓ base directory writable
    /home/you/cc-worktree/myrepo is writable
✓ [tui.keys] keymap resolves
    57 global + 116 modal binding(s) bound
...
### 7. pas de notes de worktree orphelines

Signale une note sous `<main-checkout>/.git/gwm/notes/` dont la branche n'existe plus. Ajouté par [#515](https://github.com/kbrdn1/gwm-cli/issues/515) en même temps que les notes elles-mêmes.

- **`✓`** : chaque note stockée a encore sa branche
- **`!`** : au moins une orpheline, nommée, avec le répertoire des notes comme remédiation

Avertissement et jamais Échec, et volontairement pas branché sur `gwm clean` ni `gwm remove` : qu'une note survive à son worktree est précisément l'intention (le travail n'a pas encore atterri), et elle coûte quelques centaines d'octets.

### 8. pas de config de branche orpheline

Signale les clés `branch.<name>.gwm-*` dont la branche n'existe plus. Ajouté par [#633](https://github.com/kbrdn1/gwm-cli/issues/633).

- **`✓`** : aucune clé gwm laissée derrière par une branche supprimée
- **`!`** : au moins une, comptée et nommée (au-delà de cinq branches, la liste est élidée), avec `gwm doctor --fix` comme remédiation

Contrairement aux vérifications 5 et 6, celle-ci n'est pas cosmétique. `gwm list` ouvre un `Repository` par worktree, et libgit2 reparse `.git/config` à chaque ouverture ; `statuses()` et `branch.upstream()` en prennent ensuite chacun un instantané, qui duplique toutes les entrées. Les deux coûts sont linéaires en taille du fichier, donc un listing paie pour des branches mortes depuis des mois. Mesuré à 8 worktrees constants, la taille de la config étant la seule variable : 45 ms à 13 lignes, 184 ms à 1434.

`gwm create` écrit environ sept de ces clés par branche et rien ne les enlève quand la branche disparaît, donc le fichier ne fait que grossir.

**Un dépôt qui suit la règle « ne jamais supprimer la branche source » de ce projet ne trouvera rien ici, et paiera quand même la taille du fichier à chaque listing.** Mesuré sur gwm-cli lui-même : 917 clés `gwm-*` réparties sur 193 branches locales, toutes vivantes, donc `--fix` n'en purge aucune. Les clés s'accumulent là où les branches sont supprimées, ce qui est le cas de la plupart des dépôts mais pas de celui-ci. C'est pourquoi le listing a aussi été parallélisé : la purge est le levier d'un dépôt qui élague ses branches, le parallélisme celui d'un dépôt qui ne le fait pas.

Le balayage matche le **préfixe** `branch.<name>.gwm-`, pas une liste de noms de clés connus, donc une clé ajoutée dans une version ultérieure est couverte sans que personne ait à y penser. Les clés que git écrit lui-même (`remote`, `merge`, `rebase`, …) ne sont jamais touchées, que la branche soit vivante ou morte.

#### `gwm doctor --fix`

```bash
$ gwm doctor --fix
✓ purged 869 orphan branch config key(s) from 162 deleted branch(es)
✓ .gwm.toml parses
…
```

La seule vérification avec une remédiation automatique, et opt-in parce qu'elle modifie `.git/config`. Un `gwm doctor` nu n'écrit jamais. `--fix` s'exécute avant le rapport, donc les vérifications affichées en dessous reflètent l'état réparé.

Trois choses qu'il ne fait volontairement pas :

- **Il ne travaille que sur le niveau local de la config.** Supprimer via la vue fusionnée laisserait git résoudre la clé vers le fichier qui en détient la copie la plus prioritaire, jusqu'à `~/.gitconfig`.
- **Il ne touche pas à un fichier qui ne lui appartient pas.** Une clé peut venir d'un fichier tiré par `include.path` : git résout la suppression via la vue fusionnée et annonce un succès, mais seul `.git/config` est réécrit, donc la clé survit. `--fix` relit la config après l'écriture et nomme les survivantes au lieu de les compter comme purgées. La même ligne couvre une config qui n'a pas pu être écrite du tout (verrouillée, en lecture seule), et une purge en échec ne supprime jamais le rapport : une commande de diagnostic qui perd son diagnostic parce que son remède optionnel a échoué, c'est le mauvais arbitrage.
- **Il laisse l'en-tête `[branch "…"]` vide derrière lui.** libgit2 supprime les clés, pas la section, donc une branche qui portait sept clés tombe à une ligne et non à zéro. Sur un dépôt greffé à 1434 lignes, purger 869 clés a ramené le fichier à 561 lignes, pas à 13. Ce chiffre est mesuré et non calculé par `1434 - 869` : une clé multi-valuée occupe plusieurs lignes, et les clés des branches encore vivantes ne sont pas touchées.

### 9. répertoire de base accessible en écriture
...
### 11. `worktree.branch_pattern` fait l'aller-retour via le parseur
