...
- **Il ne touche pas à un fichier qui ne lui appartient pas.** Une clé peut venir d'un fichier tiré par `include.path` : git résout la suppression via la vue fusionnée et annonce un succès, mais seul `.git/config` est réécrit, donc la clé survit. `--fix` relit la config après l'écriture et nomme les survivantes au lieu de les compter comme purgées.
