...
`cargo binstall` résout l'archive (`gwm-v{version}-{target}.tar.gz`, `.zip` sous Windows) directement depuis la Release GitHub correspondante et déballe le binaire — aucune invocation de toolchain Rust et aucune compilation de libgit2 au moment de l'installation, contrairement à `cargo install gwm` qui construit le crate localement.
