...
Méthodes : `list` (tableau de worktrees), `doctor` (rapport de santé avec `severity` + `exit_code`), `path` (`{"params":{"pattern":"<fuzzy>"}}` → `{ name, path, branch }`), et `subscribe` (transforme la connexion en flux de notifications à sens unique). Les changements de sessions d'agents poussent aussi : une session qui apparaît, disparaît ou bascule `active` ↔ `idle` déclenche un `worktrees.changed`, tandis que le seul tic de son horodatage `last_activity` reste silencieux (même règle que `age_seconds`). Les schémas de réponse sont les mêmes DTO stables que ceux émis par les flags CLI `--format=json` — voir [`docs/schema/`](https://github.com/kbrdn1/gwm-cli/tree/main/docs/schema) et la [référence `gwm daemon`](/fr/cli/reference#gwm-daemon-socket-path-poll-ms-ms-issue-38).
