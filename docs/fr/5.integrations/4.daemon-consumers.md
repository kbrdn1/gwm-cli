...
> Le daemon est **un par dépôt** — il répond pour le dépôt dans lequel il a été lancé. Lancez-le depuis l'ensemble de worktrees que vous voulez surveiller (`cd` dans n'importe quel worktree du dépôt, puis `gwm daemon`).
...
zsh — prompt de droite (rafraîchi à chaque prompt ; peu coûteux car le daemon détient déjà l'état — un aller-retour socket, pas de scan git) :
...
set -ag status-right ' #(cd #{pane_current_path} && gwm statusline)'
```
...
printf '{"jsonrpc":"2.0","method":"list","id":1}\n' \
  | socat - UNIX-CONNECT:"$XDG_RUNTIME_DIR/gwm.sock"

# nc / netcat
printf '{"jsonrpc":"2.0","method":"list","id":1}\n' \
  | nc -U "$XDG_RUNTIME_DIR/gwm.sock"
...
[
  ...
  },
  {
    ...
    "command": "printf '{\"jsonrpc\":\"2.0\",\"method\":\"list\",\"id\":1}\\n' | socat - UNIX-CONNECT:\"$XDG_RUNTIME_DIR/gwm.sock\" | jq -r '.result[] | \"\\(.name)\\t\\(.path)\"'",
    "reveal": "always"
  }
]
```
  ...
  "tasks": [
    {
      "label": "gwm: worktrees (via daemon)",
      "type": "shell",
      "command": "printf '{\"jsonrpc\":\"2.0\",\"method\":\"list\",\"id\":1}\\n' | socat - UNIX-CONNECT:\"${env:XDG_RUNTIME_DIR}/gwm.sock\" | jq -r '.result[] | \"\\(.name)\\t\\(.path)\"'",
      "problemMatcher": []
    }
  ]
}
...
printf '{"jsonrpc":"2.0","method":"path","params":{"pattern":"309"},"id":1}\n' \
  | socat - UNIX-CONNECT:"$XDG_RUNTIME_DIR/gwm.sock" | jq -r '.result.path'
```

Sans daemon en cours, les mêmes données sont à une invocation `gwm` près — `gwm list --format=json` et `gwm path <pattern> --format=json` émettent les schémas identiques — mais le daemon évite un spawn de processus (et un nouveau scan git) par requête.
