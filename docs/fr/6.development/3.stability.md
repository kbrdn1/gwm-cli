...
Trois de ces surfaces sont gelées *mécaniquement*, et un renommage casse la CI
...
majeur, parmi eux le champ `repo` propre au mode workspace sur une ligne `list` et
...
- [`docs/schema/README.md`](https://github.com/kbrdn1/gwm-cli/blob/main/docs/schema/README.md)
  pour les paliers stable/expérimental par champ et le contrat de détection de
  dérive.
- [`src/contract.rs`](https://github.com/kbrdn1/gwm-cli/blob/main/src/contract.rs)
  pour la source de vérité unique sur `SCHEMA_VERSION` et les ensembles gelés de
  méthodes/sections.
- [Contribuer → Releases](/fr/development/contributing) et
  ...
  pour le processus de release SemVer et le workflow de tag.
