...
- User wants to **write down where they were** on a worktree: `N` in the TUI opens its note in `$EDITOR` (#515), `gwm note show` reads it back, `gwm doctor` reports the ones whose branch is gone.
...
| `N`             | open the selected worktree's note in `$EDITOR` (#515); `≡` marks the rows that have one |
